...
class RotatingTriangle(QObject):
    ...
    def set_angle(self, value):
        if value != self._angle:
            self._angle = value
            self.parent().update()  # Actualiza el botón para redibujar el triángulo
...
class CustomTriangleButton(QPushButton):
    def __init__(self, text, parent=None):
        ...
        # Crear el objeto para manejar la rotación
        self.rotating_triangle = RotatingTriangle(self)
        
        # Inicializar la animación
        self.animation = QPropertyAnimation(self.rotating_triangle, b"angle")
        self.animation.setDuration(70)  # Duración de la animación en milisegundos (más rápida)
        self.animation.setEasingCurve(QEasingCurve.Linear)  # Usar QEasingCurve.Linear
    ...
    def paintEvent(self, event):
        ...
        points = [
            QPoint(x_pos, y_pos + triangle_size),  # Vértice inferior izquierdo
            QPoint(x_pos + triangle_size, y_pos + triangle_size),  # Vértice inferior derecho
            QPoint(x_pos + triangle_size / 2, y_pos)  # Vértice superior
        ]
        triangle = QPolygon(points)

        # Configurar el pincel y el color del triángulo
        brush = QBrush(QColor(255, 255, 255))
        painter.setBrush(brush)
        painter.setPen(Qt.NoPen)

        # Aplicar la rotación del triángulo
        transform = QTransform()
        transform.translate(x_pos + triangle_size / 2, y_pos + triangle_size / 2)  # Mover el centro de rotación
        transform.rotate(self.rotating_triangle.angle)
        transform.translate(-(x_pos + triangle_size / 2), -(y_pos + triangle_size / 2))  # Mover el centro de vuelta
